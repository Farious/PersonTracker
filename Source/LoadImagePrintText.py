...
def loadImagePrintText(cam, frame, debugREID = 1):
    ...
    for i, values in enumerate(res1):
        res1[i] = [int(float(value)) for value in values]
        left = res1[i][0]
        top = res1[i][1]
        right = left+res1[i][2]
        bottom = top+res1[i][3]
        ...
        if len(res1[i]) > 5:  # There is a re-IDentification for this detection
            ...
            for k, ID in enumerate(REIDs):
                ...
                cv2.rectangle(image, (left, top-textHeight*j),
                ...
                if debugREID == 0:
                    color = black
                cv2.putText(image, text, (left, top-textHeight*j), cv2.FONT_HERSHEY_DUPLEX, fontScale, color, thickness=thickness/2)
